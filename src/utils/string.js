...
demuxes.s = function(
  ...
  if (flags.indexOf('-', 0) > -1) {
    ...
    replacement = repeat(' ', Number(width) - replacement.length) +
      replacement;
  }
  return replacement;
};
 ...
 * 'Cost: ' + cost.toFixed(2).
 // * @param {string} template Template string containing % specifiers.
...
export function format(template, args) {
  ...
  function replacerDemuxer(
    ...
    return demuxes[type].apply(null, arguments);
  }
...
export function repeat(char, count, sep) {
  if (sep) {
    return slice(repeat(char+sep, count, null), 0, -1);
  }
